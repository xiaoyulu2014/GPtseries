MGP2 = function(x,y,N,Q,num_gen,lag,max,min,Pc,Pm,x_test){ 
...
#     y = dat[(1+lag+1):(N+lag+1)]
  ...
  #prediction
  x_test = matrix(dat[(N+2):(N+lag+1)],lag,1)
  mean_pos = cbind(t(x_test),rep(1,1)) %*% w_best
  ...
  return(y_hat) #fitted value from N+lag+2 to N+lag+M+1 for M-step ahead prediction
}
