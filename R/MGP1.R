MGP = function(dat,N,Q,lag,max,min,Pc,Pm,x_test,step){ 
...
#     for (i in 1:Q){
    M = foreach(i=1:Q, .combine='cbind') %dopar%   {
      tmp0 = matrix(,N,N)
      for (j in 1:N){
        for (k in 1:N){
          tmp0[j,k] = theta[i,1]^2*exp(-sum((x[,j]-x[,k])^2)/(2*theta[i,2]^2))
        }
      }
      tmp0 = tmp0 + theta[i,3]^2*diag(N)
      return(tmp0)
    }
      
    w = foreach(i=1:Q, .combine='rbind') %dopar%   {
      tmp1 = M[1:N,((i-1)*N+1):(i*N)]
      cK = chol(tmp1)
      iK = chol2inv(cK)
      return(t(solve(t(x_tilda)%*%iK%*%x_tilda)%*%t(x_tilda)%*%tmp1%*%y))
    }
      
    fitness = foreach(i=1:Q, .combine='c') %dopar%   {
      tmp1 = M[1:N,((i-1)*N+1):(i*N)]
      cK = chol(tmp1)
      iK = chol2inv(cK)
      return(-(0.5*2*sum(log(diag(cK))) + 0.5*t(y-x_tilda%*%w[i,])%*%iK%*%(y-x_tilda%*%w[i,]) + N/2*log(2*pi)))
    }
  ...
  w_best = w[best,]
  K_best = M[1:N,((best-1)*N+1):(best*N)]
  ...
  for (i in 1:ncol(x_test)){
    for (j in 1:N){
      Sigma_pos[i,j] = theta[1]^2*exp(-sum((x_test[,i]-x[,j])^2)/(2*theta_best[2]^2))
    }
  }
